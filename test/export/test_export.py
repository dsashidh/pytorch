...
def forward(self, p_linear_weight, p_linear_bias, b_buffer, x):
    ...
    def test_export_aten_to_unflatten(self):
        ...
        ep = export(Foo(), (inp,), strict=False, preserve_module_call_signature=("bar",))
        mod = ep.module()
    ...
    def test_export_aten_to_unflatten_subclass(self):
        ...
        class Foo(torch.nn.Module):
            def __init__(self):
                super().__init__()
                self.bar = Bar() 
                self.param = torch.nn.Parameter(TwoTensor(torch.ones(4, 4), torch.ones(4, 4)))
        ...
        with self.assertRaisesRegex(ValueError, "It looks like p_param is a tensor subclass."):
            export(Foo(), (inp,), strict=False, preserve_module_call_signature=("bar",)).run_decompositions({})
    ...
    def test_export_aten_to_unflatten_subclass_pre_dispatch(self):
        ...
        ep = export_for_training(Foo(), (inp,), strict=False, preserve_module_call_signature=("bar",))
